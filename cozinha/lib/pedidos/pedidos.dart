...
class _TablePedidosState extends State<TablePedidos> {
  ...
  List<Map<String, dynamic>> _pedidosEmPreparo = [];
  ...
  void _listaPedidosEmPreparo() async {
    _pedidosEmPreparo = [];
    final data = await PedidoService.listaPedidosEmPreparo();
    setState(() {
      for (var p in data) {
        _pedidosEmPreparo.add(<String, dynamic>{
          "id": p.id,
          "idMesa": p.idMesa,
          "prato": p.prato,
          "status": p.status
            ...
            Container(
              child: _carregando
                  ...
                  : Padding(
                      padding: const EdgeInsets.all(16),
                      child: ListView.builder(
                        itemCount: _pedidosEmPreparo.length,
                        itemBuilder: (context, index) => Card(
                          margin: const EdgeInsets.all(15),
                          child: ListTile(
                            title: Text(
                              (_pedidosEmPreparo[index]['prato'].toString()),
                              style: const TextStyle(color: Colors.white),
                            ),
                            subtitle: Text(
                              'Mesa: ${_pedidosEmPreparo[index]['idMesa']}',
                              style: const TextStyle(color: Colors.white),
                            ),
                            trailing: SizedBox(
                              width: 100,
